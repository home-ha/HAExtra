...
# [key1, key2, ...] or {key1:value1, key2:value2, ...}
# [cool, heat, ...] or {cool:2, heat:4, ...}
def keys_of_list_or_dict(collection):
    return collection.all_keys() if isinstance(collection, dict) else collection

def key_of_list_or_dict(collection, value):
    if value is not None:
        if isinstance(collection, list):
            ...
            for k, v in collection.items():
                if v == value:
                    return k
    _LOGGER.error("Invalid value %s with %s", value, collection)
    return None

def value_of_list_or_dict(collection, key):
    if isinstance(collection, list):
        if value in collection:
            ...
            if value in collection:
                return collection[key]
    _LOGGER.error("Invalid key %s with %s", key, collection)
    raise ValueError
...
class ModbusClimate(ClimateDevice):
    ...
    def current_operation(self):
        ...
        return key_of_list_or_dict(ModbusClimate._operation_list, operation)

    @property
    def operation_list(self):
        """Return the list of available operation modes."""
        return keys_of_list_or_dict(ModbusClimate._operation_list)

    @property
    def current_fan_mode(self):
        """Return the fan setting."""
        fan = self.get_value(CONF_FAN)
        return key_of_list_or_dict(ModbusClimate._fan_list, fan)

    @property
    def fan_list(self):
        """Return the list of available fan modes."""
        return keys_of_list_or_dict(ModbusClimate._fan_list)

    @property
    def current_swing_mode(self):
        """Return the swing setting."""
        swing = self.get_value(CONF_SWING)
        return key_of_list_or_dict(ModbusClimate._swing_list, swing)

    @property
    def swing_list(self):
        """List of available swing modes."""
        return keys_of_list_or_dict(ModbusClimate._swing_list)
    ...
    def set_operation_mode(self, operation_mode):
        ...
        try:
            ...
            if is_on:
                ...
                value = value_of_list_or_dict(self.self._operation_list, operation_mode)
                self.set_value(CONF_OPERATION, value)
        except ValueError:
    ...
    def set_fan_mode(self, fan_mode):
        """Set new fan mode."""
        try:
            value = value_of_list_or_dict(self._fan_list, fan_mode)
            self.set_value(CONF_FAN, value)
        except ValueError:
    ...
    def set_swing_mode(self, swing_mode):
        """Set new swing mode."""
        try:
            value = value_of_list_or_dict(self._swing_list, swing_mode)
            self.set_value(CONF_SWING, value)
        except ValueError:
